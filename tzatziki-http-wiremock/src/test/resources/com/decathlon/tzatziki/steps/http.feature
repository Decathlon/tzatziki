Feature: to interact with an http service and setup mocks
  ...
  Scenario: we can define the assertion type for the received payload
    ...
    And "http://backend/users" has received a POST and at least:
      ...
      name: bob
      """
#    And "http://backend/users" has received a POST and only: TODO: Check what it means
#      """yml
#      id: 1
#      """
  ...
  Scenario: we can assert that we received a get on an url with queryParams and a capture group //TODO (\d+) not well parsed by UrlEncodeUtils
  ...
  @ignore
  Scenario: we can assert a complex request in one line //TODO to correct akward after 100ms make bug
