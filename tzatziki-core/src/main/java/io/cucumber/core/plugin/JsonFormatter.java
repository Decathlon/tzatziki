...
public final class JsonFormatter implements EventListener {
    ...
    private void finishReport(TestRunFinished event) {
        ...
        // ↓ this block will remove the steps that we skipped on purpose
        featureMaps.forEach(feature -> ((List<Map<String, Object>>) feature.get("elements")).forEach(element -> {
            List<Map<String, Object>> steps = (List<Map<String, Object>>) element.get("steps");
            for (int i = 0; i < steps.size(); i++) {
                if (!steps.get(i).containsKey("result") || isSkippedGuard(steps.get(i))) {
                    steps.remove(i);
                    i--;
                }
            }
    ...
    // ↓ required for the block above to work
